/**
  * This sketch demonstrates how to use the <code>getMidLabel</code> method of a <code>FloatControl</code> object.
  ...
  * is used for sending audio to, or receiving audio from, the audio system. Float controls have a maximum and minimum 
  * value that they can be set to. The midpoint of this range has an associated <code>String</code> label, 
  * which is what is returned by the <code>getMidLabel</code> method.
  */
...
void draw()
{
  ...
  {
    text("The pan's midpoint value label is " + out.pan().getMidLabel() + ".", 5, 15);
  }
  else
  {
