/**
  ...
  * want the buffer size of the returned <code>AudioSample</code> to be. Minim is able to load wav files, au files, aif
  * files, snd files, and mp3 files. When you call <code>loadSample</code>, if you just specify the filename it will try 
  * to load the sample from the data folder of your sketch. However, you can also specify an absolute path (such as 
  * "C:\foo\bar\thing.wav") and the file will be loaded from that location (keep in mind that won't work from an applet). 
  * You can also specify a URL (such as "http://www.mysite.com/mp3/song.mp3") but keep in mind that if you run the sketch 
  * as an applet you may run in to security restrictions if the applet is not on the same domain as the file you want to load. 
  * You can get around the restriction by signing all of the jars in the applet. Before you exit your sketch make sure you call the 
  * <code>close</code> method of any <code>AudioSamples</code>'s you have received from <code>loadSample</code>.
  * <p>
