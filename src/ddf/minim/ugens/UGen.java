...
public abstract class UGen
{
  ...
  // ddf: UGen class members are before the UGenInput definition because the UGenInput class 
  // refers to some of these. I think it's clearer to see these before reading the 
  // UGenInput code.
  ...
  private int m_currentTick;
	...
	public final class UGenInput
	{
      ...
      /**
       * Returns the first value in the array of last values. This is meant to make code that gets values 
       * from control inputs easier to read.
       */
      ...
      /**
       * Sets all values in the last values array to the provided value. If you want to set last values 
       * in the different channels of this input to different values, you should use getLastValues to 
       * do so. For example:
       * <pre>
       ...
       * </pre>       
       * 
       * @param value the value to set all last values to
       */
      ...
      // this will be called by the owning UGen *only* when something is patched to this input.
      void tick()
      {
        if ( m_incoming != null )
        {
	...
	public final UGen patch(UGen connectToUGen)
	{
		...
		// TODO jam3: m_nOutputs should only increase when this chain will be ticked!
		m_nOutputs += 1;
	...
	public final void unpatch(UGen connectToUGen)
	{
		...
		// TODO m_nOutputs needs to be updated as the convers of patch above.
		m_nOutputs -= 1;
		Minim.debug("m_nOutputs = " + m_nOutputs);
	}
