...
public interface AudioRecording extends AudioResource
{
   ...
   * 
   * @param count
   *          the number of times playback should loop back from the loop's end
   *          position to the loop's start position, or Minim.LOOP_CONTINUOUSLY
  ...
  /**
   * How many loops are left to go. 0 means this isn't looping and -1 means that
   * it is looping continuously.
   * 
  ...
  /**
   * Returns the length of the source in milliseconds. Infinite sources, such as
   * internet radio streams, should return -1.
   * 
  ...
  /**
   * Returns a Map of properties, such as length, format, ID3 tags perhaps.
   * 
   * @return a Map of properties of the source
   */
  Map getProperties();
}
